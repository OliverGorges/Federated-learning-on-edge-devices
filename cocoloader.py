...
annotations = os.path.join(dataDir, "cocoTrain25082020.json")
...
def writeData(i, folder):
    ...
    print(name)
    print(name[:-4])
    a = {'_id': name[:-4], 'image': f"{name}", 'thermalImage': f"{name}", 'height': i['image']['height'],'width': i['image']['width'],'objects': i['objects']}
