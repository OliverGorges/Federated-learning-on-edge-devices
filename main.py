...
def thermalImageThread():
    ...
    t2 = time.time()
    #predict faces
    faceDetection1.prepareImage(thermalImg, 1)
    faces = faceDetection1.detectFace()
    # Add data to Q
    data_queue.append((thermalImg, faces, image_data))
    logging.debug(f"Runtime ThermalThread: {time.time() - t1} / Image {t2 - t1} / Prediction {time.time() - t2}")
...
def imageThread(frame):
    ...
    faceDetection2.prepareImage(frame, m)
    faces = faceDetection2.detectFace()
...
with ThreadPoolExecutor(max_workers=4) as executor:
    while True:
        ...
        if ImageThread is None:
            ImageThread = executor.submit(imageThread, liveFrame)
        if ImageThread.done():
